# Bayesian 3-state survival regression for screening data
# See demo.r for a vignette
bts_survreg = function(d=NULL, L=NULL, R=NULL, Z.X = NULL, Z.S = Z.X, parallel = T,
  ...
  run = foreach(j = 1:chains # ndraws=rep(mc, chains)+1:chains
                , .export = c('pst.X', 'r.trdist', 'pdist', 'qdist', 'pst.S', 'rdist',
                              ...
                              'trans.par.norm', 'cor2cov',
                              'logrob', 'trans.par.ind.norm',
                              'mhstep.aft.XS', 'pst.aft.XS')
  ) %myinfix% {
    library(MCMCpack)
    library(mvtnorm)
    set.seed(s[j])
    ...
    # Begin Gibbs - find starting values that are valid
    go = 0
    ac.X = ac.S = 1
    while(go == 0){
      # Init
  ...
  # Bind to previous run if updating
  if(!is.null(prev.run)){
